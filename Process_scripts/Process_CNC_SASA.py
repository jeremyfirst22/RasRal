...
for index,mol in enumerate(molecList) : 
    ...
    datafile = '%s/Analysis/boltzmann/cnc_area.weighted.out'%molec
    #print datafile
    ...
    #index +=1
fig.savefig('figures/bar_sasa_cnc.png',format='png') 
plt.close() 
...
fig.savefig('figures/peak_v_cnc_sasa.png',format='png') 

fig, ax = plt.subplots(1,1) 
fig.subplots_adjust(wspace=0.1,hspace=0.35,left=0.15,right=0.95) 
fig.text(0.5,0.04, r"SASA Q61X", ha='center', va='center') 
fig.text(0.03,0.5, r"SASA CNC ($\rm{\AA}^2$)", ha='center', va='center',rotation='vertical') 
...
for mol in molecList : 
    ...
    datafile2 = '%s/Analysis/boltzmann/cnc_area.weighted.out'%molec
    try : 
        with open(datafile) as f : 
            lines = f.readlines() 
            avg = float(lines[2].split()[-2]) 
    ...
    peak, error = avg2, std2 #nameToExpPeak[name] 
...
fig.savefig('figures/sasa_q61_v_cnc_sasa.png',format='png') 
