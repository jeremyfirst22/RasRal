...
for index,mol in enumerate(molecList) : 
    ...
    datafile = '%s/Analysis/boltzmann/frame_hb.weighted.out'%molec
    #print datafile
    
    try : 
        with open(datafile) as f : 
            lines = f.readlines() 
            avg = float(lines[6].split()[-2]) 
            std = float(lines[6].split()[-1])
    except IOError : 
    ...
    #index +=1
fig.savefig('figures/bar_hbond_cnc.png',format='png') 
plt.close() 
...
fig.savefig('figures/peak_v_cnc_hbond.png',format='png') 
