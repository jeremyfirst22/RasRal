...
fig.text(0.03,0.5, r"RMSD of GTP heavy atoms ($\rm{\AA}$)", ha='center', va='center',rotation='vertical') 

avgAccum, rateAccum = [], [] 
for mol in molecList : 
    molec = 'RasRalC18CNC_Q61%s'%mol
    name = "Q61%s"%mol 
    datafile = '%s/Analysis/boltzmann/rmsd.weighted.out'%molec
...
fig.savefig('figures/rate_v_rmsd_gtp.png',format='png') 
plt.close() 

fig, ax = plt.subplots(1,1) 
fig.subplots_adjust(wspace=0.1,hspace=0.35,left=0.15,right=0.95) 
fig.text(0.5,0.04, r"Rate (min$^{-1}$)", ha='center', va='center') 
fig.text(0.03,0.5, r"RMSD of GTP active phosphate and MG ($\rm{\AA}$)", ha='center', va='center',rotation='vertical') 

avgAccum, rateAccum = [], [] 
for mol in molecList : 
    molec = 'RasRalC18CNC_Q61%s'%mol
    name = "Q61%s"%mol 
    datafile = '%s/Analysis/boltzmann/active.weighted.out'%molec
    color,marker = plotKeys[mol] 
    #print datafile
    try : 
        with open(datafile) as f : 
...
fig.savefig('figures/rate_v_rmsd_active.png',format='png') 
plt.close() 
